...
#'
#' Based on the extent or polygon of a provided study area, downloads overlapping tiles from Global Forest Watch database
#' 2024-v1.12 (Hansen et al. 2013), including tree canopy cover in year 2000 and forest cover loss per year until 2024,
#' and exports processed raster layers.
#'
...
#' @param filenames Character vector of length 2, indicating the name for the outputs of tree cover
#' and loss (raster layers). Default is `c("tree_cover", "loss_year")`.
...
#' @param ... Other arguments passed to `curl::multi_download`.
#'
#' @details
#' The function uses `curl::multi_download` to download multiple files (tiles) concurrently.
#' The downloaded tiles include:
#' (1) a raster layer of tree canopy cover in year 2000;
#' (2) a raster layer of forest cover loss for subsequent years up to 2024. The loss layer is
#' encoded as 0 (no loss) or as integer values (1, 2, ..., 24) corresponding to loss detected primarily in years 2001, 2001, ..., 2024.
#'
#' Tiles of the same variable (tree canopy cover or forest loss) are merged and cropped to the
#' study area extent specified in `study_area`. If `mask = TRUE`, the tiles will be masked to
#' the provided extent or boundaries of the polygon.
#'
#' @return Processed raster layers from GFW databases, one for tree canopu cover
#' and another for forest cover loss per year, based on the provided study area.
#'
#' @references
...
get_gfw <- function(study_area,
  ...
  links <- rbind(treecover2000_links, treeloss_links)

  # Download
  fnames <- substring(links$link, first = 74)
  downloads <- curl::multi_download(
    urls = links$link,
    destfiles = paste0(normalizePath(tmp, winslash = "/"), "/", fnames),
    progress = !silent, ...)

  if(!any(downloads$success)){
    unlink(file.path(tmp), recursive = T)
    message("One or more downloads failed. See details below:")
    return(downloads)
  }
...
#   }
  ...
  if(!silent) message("Raster layers processed and exported successfully.")
}
