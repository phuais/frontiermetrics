...
A key parameter named `onset_min` of the onset metric can also be changed. By default, onset is defined as the first year in the time series with at least 3 cells showing forest loss within a frontier. This criterion can be made more stringent by increasing the required number of cells. For example, by setting `onset_min = 5`. Let's run `fmetrics()` with these new parameters:
